...
//fsi template specific handling of navigation component
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let cmp_navigation_link_click_listener = function (event) {
  ...
  for (let link of links) {
    let url = new URL(link.href);
    if (current_page.pathname === url.pathname) {
      $(link).css({ color: "var(--primary-color)" });
      $(link)
        .parentsUntil(".navigation", "ul.cmp-navigation__group")
        .closest("li.cmp-navigation__item")
        .addClass("expanded");
    }
  }
  ...
  cmpNav.addEventListener("pointerup", function (event) {
    event.preventDefault();
    event.stopPropagation();
    if (
      event.target &&
      event.target.matches &&
      event.target.matches(cmp_navigation_links_selector)
    ) {
      //handle exact click
      cmp_navigation_link_click_listener(event);
    } else if (
      event.target &&
      event.target.closest &&
      event.target.closest(cmp_navigation_links_selector)
    ) {
      //handle click on a child element of the given selector - such a span.icon inside an anchor tag
      cmp_navigation_link_click_listener(event);
    }
  });

  //listen to keyup rather than keydown to capture the latest value after keystroke
  $(".navigation input[name='cmp-navigation-filter']").on(
    "keyup",
    function (event) {
      let $input = $(this);
      let filter = $input.val();
      ...
      $navigation
        .find(
          "ul.cmp-navigation__group > li.cmp-navigation__item > a, ul.cmp-navigation__group > li.cmp-navigation__item > span"
        )
  ...
  $(
    ".navigation .cmp-navigation-filter-container .cmp-navigation-filter__closeicon"
  ).on("click", function () {
    let $navigation = $(this).closest(".navigation");
